...
CREATE TABLE mega_table (
    ...
    date_first_observed INT(10), # could cast to date, mostly 0's though so could be dropped. Perhaps replace 0's with issue date?
    law_section INT(4),
    ...
    vehicle_year INT(4), #25% are 0 for this, maybe cast to NULL
    meter_number VARCHAR(100), # MOSTLY NULL, we can drop this
    feet_from_curb INT(10), # almost all 0's, could also drop
    violation_post_code VARCHAR(10), # could cast to INT after replacing empties with 0
    violation_description VARCHAR(50), # replace NULL with 'unspecified' or fill using rule
